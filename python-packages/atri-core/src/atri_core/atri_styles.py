...
class AtriStyles:
    def __init__(self, state: Optional[Any]):
        ...
        self.display: Optional[str] = state["display"] if state != None and "display" in state else None
        self.flexDirection: Optional[str] = (
            state["flexDirection"] if state != None and "flexDirection" in state else None
        )
        self.alignItems: Optional[str] = state["alignItems"] if state != None and "alignItems" in state else None
        self.justifyContent: Optional[str] = (
            state["justifyContent"] if state != None and "justifyContent" in state else None
        )
        self.flexWrap: Optional[str] = state["flexWrap"] if state != None and "flexWrap" in state else None
        self.alignContent: Optional[str] = (
            state["alignContent"] if state != None and "alignContent" in state else None
        )
        self.rowGap: Optional[str] = state["rowGap"] if state != None and "rowGap" in state else None
        self.columnGap: Optional[str] = state["columnGap"] if state != None and "columnGap" in state else None
        self.alignSelf: Optional[str] = state["alignSelf"] if state != None and "alignSelf" in state else None
        self.flexGrow: Optional[str] = state["flexGrow"] if state != None and "flexGrow" in state else None
        self.flexShrink: Optional[str] = state["flexShrink"] if state != None and "flexShrink" in state else None
        self.order: Optional[str] = state["order"] if state != None and "order" in state else None
        self.marginTop: Optional[str] = state["marginTop"] if state != None and "marginTop" in state else None
        self.marginBottom: Optional[str] = (
            state["marginBottom"] if state != None and "marginBottom" in state else None
        )
        self.marginRight: Optional[str] = state["marginRight"] if state != None and "marginRight" in state else None
        self.marginLeft: Optional[str] = state["marginLeft"] if state != None and "marginLeft" in state else None
        self.paddingTop: Optional[str] = state["paddingTop"] if state != None and "paddingTop" in state else None
        self.paddingBottom: Optional[str] = (
            state["paddingBottom"] if state != None and "paddingBottom" in state else None
        )
        self.paddingRight: Optional[str] = (
            state["paddingRight"] if state != None and "paddingRight" in state else None
        )
        self.paddingLeft: Optional[str] = state["paddingLeft"] if state != None and "paddingLeft" in state else None
        self.width: Optional[str] = state["width"] if state != None and "width" in state else None
        self.height: Optional[str] = state["height"] if state != None and "height" in state else None
        self.minWidth: Optional[str] = state["minWidth"] if state != None and "minWidth" in state else None
        self.minHeight: Optional[str] = state["minHeight"] if state != None and "minHeight" in state else None
        self.maxWidth: Optional[str] = state["maxWidth"] if state != None and "maxWidth" in state else None
        self.maxHeight: Optional[str] = state["maxHeight"] if state != None and "maxHeight" in state else None
        self.overflow: Optional[str] = state["overflow"] if state != None and "overflow" in state else None
        self.fontFamily: Optional[str] = state["fontFamily"] if state != None and "fontFamily" in state else None
        self.fontWeight: Optional[str] = state["fontWeight"] if state != None and "fontWeight" in state else None
        self.fontSize: Optional[str] = state["fontSize"] if state != None and "fontSize" in state else None
        self.textAlign: Optional[str] = state["textAlign"] if state != None and "textAlign" in state else None
        self.color: Optional[str] = state["color"] if state != None and "color" in state else None
        self.opacity: Optional[str] = state["opacity"] if state != None and "opacity" in state else None
        self.fontStyle: Optional[str] = state["fontStyle"] if state != None and "fontStyle" in state else None
        self.borderRadius: Optional[str] = (
            state["borderRadius"] if state != None and "borderRadius" in state else None
        )
        self.borderWidth: Optional[str] = state["borderWidth"] if state != None and "borderWidth" in state else None
        self.borderStyle: Optional[str] = state["borderStyle"] if state != None and "borderStyle" in state else None
        self.borderColor: Optional[str] = state["borderColor"] if state != None and "borderColor" in state else None
        self.backgroundImage: Optional[str] = (
            state["backgroundImage"] if state != None and "backgroundImage" in state else None
        )
        self.backgroundColor: Optional[str] = (
            state["backgroundColor"] if state != None and "backgroundColor" in state else None
        )
        self.backgroundClip: Optional[str] = (
            state["backgroundClip"] if state != None and "backgroundClip" in state else None
        )
        self.backgroundOrigin: Optional[str] = (
            state["backgroundOrigin"] if state != None and "backgroundOrigin" in state else None
        )
        self.backgroundAttachment: Optional[str] = (
            state["backgroundAttachment"] if state != None and "backgroundAttachment" in state else None
        )
        self.backgroundPositionX: Optional[str] = (
            state["backgroundPositionX"] if state != None and "backgroundPositionX" in state else None
        )
        self.backgroundPositionY: Optional[str] = (
            state["backgroundPositionY"] if state != None and "backgroundPositionY" in state else None
        )
        self.backgroundRepeat: Optional[str] = (
            state["backgroundRepeat"] if state != None and "backgroundRepeat" in state else None
        )
        self.position: Optional[str] = state["position"] if state != None and "position" in state else None
        self.float: Optional[str] = state["float"] if state != None and "float" in state else None
        self.clear: Optional[str] = state["clear"] if state != None and "clear" in state else None
        self.top: Optional[str] = state["top"] if state != None and "top" in state else None
        self.left: Optional[str] = state["left"] if state != None and "left" in state else None
        self.bottom: Optional[str] = state["bottom"] if state != None and "bottom" in state else None
        self.right: Optional[str] = state["right"] if state != None and "right" in state else None
        self.zIndex: Optional[str] = state["zIndex"] if state != None and "zIndex" in state else None
        self.outlineStyle: Optional[str] = (
            state["outlineStyle"] if state != None and "outlineStyle" in state else None
        )
        self.outlineColor: Optional[str] = (
            state["outlineColor"] if state != None and "outlineColor" in state else None
        )
        self.outlineOffset: Optional[str] = (
            state["outlineOffset"] if state != None and "outlineOffset" in state else None
        )
        self.outlineWidth: Optional[str] = (
            state["outlineWidth"] if state != None and "outlineWidth" in state else None
        )
        self.cursor: Optional[str] = state["cursor"] if state != None and "cursor" in state else None
        self.boxSizing: Optional[str] = state["boxSizing"] if state != None and "boxSizing" in state else None
